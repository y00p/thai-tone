...
class ThaiAnalysis(ts.LetterProperties):

    def __init__(self, lp):
        self.lp = lp

    def determine_tone( self, syllable, verbose = False ):
        ...
        is_closed = (len([ i for i in syl if i in self.lp.consonants]) > 1
                     and syl[-1] in self.lp.consonants)

        # does syllable have a short vowel?
        is_short  = (len([ i for i in syl if i in self.lp.shortener]) > 0
                     and 'ๅ' not in syl )

        # last consonant is a plosive?
        has_plosive_ending = False
        if is_closed and dict(zip(self.lp.consonants, self.lp.consonant_plosives))[syl[-1]]:
        ...
        found_tone_marks = [ i for i in syl if i in self.lp.tone_marks ]
        if len(found_tone_marks) == 0:
            tone_mark_index = 0
        elif len(found_tone_marks) == 1:
            tone_mark_index = self.lp.tone_marks.index(found_tone_marks[0])+1
        else:
        ...
        if syl[0] in self.lp.consonants:
            first_consonant = syl[0]
        elif syl[1] in self.lp.consonants:
            first_consonant = syl[1]
        else:
            raise ValueError('None of the first two characters is a consonant.\n',
                              errmsg_notThai)
        consonant_class = ts.Tone(dict(zip(self.lp.consonants,
                                           self.lp.consonant_classes))[first_consonant])
